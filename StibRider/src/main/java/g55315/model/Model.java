...
public class Model implements Observable {
     ...
     * @param name the id of the favorite to get
     * @return the favorite with the given name
     * @throws RepositoryException
     */
    public FavoriteDto getFavorite(String name) throws RepositoryException {
        int id = 0;
        for(FavoriteDto fav: favoritesRepository.getAll()) {
    ...
    public void insertFavoite(String begin, String end, String name) throws RepositoryException {
            favoritesRepository.add(new FavoriteDto(0,name,begin,end));
            notifyObservers();
    }
    /**
     * updates the favorite with name
     * @param name the name of the favorite to update
     * @param origin the new origin of the favorite
     * @param destination the new destination of the favorite
     * @param newName the new name of the favorite
     */
    public void updateFavorite(String name, String origin, String destination, String newName) throws RepositoryException {
        int id = 0;
        for(FavoriteDto fav: favoritesRepository.getAll()) {
            if(fav.getName().equals(name)) {
                id = fav.getKey();
        ...
        favoritesRepository.add(new FavoriteDto(id,newName,origin,destination));
        notifyObservers();
    }
